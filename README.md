...
| **6** | Training | Scripted and running correctly\* | `scripts/train_tgfem.py` — all variants and both protocols verified working end to end; a real 150-epoch run still needs a GPU |
| **7** | Evaluation & ablations | Scripted and running correctly\* | `scripts/run_ablations.py`, `scripts/eval_tgfem.py`, `scripts/eval_deepcrack.py` — all 6 priority variants, both protocols, and both eval scripts verified end to end |
| **8** | Report & demo | Working\* | `scripts/demo.py` verified end to end against a trained checkpoint; needs real numbers from Phase 6 to be useful for the report |
